...
void BilateralFilterCpuForward_3d(torch::Tensor inputTensor,
                ...
                for (int y = 0; y < desc.sizes[1]; y++) {
                    for (int x = 0; x < desc.sizes[0]; x++) {
                            ...
                            if (flagNotClamped) {
                                for (int i = 0; i < desc.channelCount; i++) {
                                    ...
                                    dfilter_dx_ki +=
                                            (-1) * totalWeight *
                                            inputTensorData[neighbourOffset + i * desc.channelStride] *
                                            colorDistance / (colorSigma *
                                                             colorSigma); // Be careful, the +1 is missing here -> Added before filling dfilter_dx_kiData

                                    colorSum_w +=
                                            totalWeight * colorDistanceSquared /
                                            std::abs(colorSigma * colorSigma * colorSigma);
                                    colorSum_alpha +=
                                            totalWeight * inputTensorData[neighbourOffset + i * desc.channelStride] *
                                            colorDistanceSquared / std::abs(colorSigma * colorSigma * colorSigma);

                                    xSum_w +=
                                            totalWeight * xDistanceSquared[kernelIndex[0]] /
                                            std::abs(sigma_x * sigma_x * sigma_x);
                                    xSum_alpha +=
                                            totalWeight * inputTensorData[neighbourOffset + i * desc.channelStride] *
                                            xDistanceSquared[kernelIndex[0]] / std::abs(sigma_x * sigma_x * sigma_x);

                                    ySum_w +=
                                            totalWeight * yDistanceSquared[kernelIndex[1]] /
                                            std::abs(sigma_y * sigma_y * sigma_y);
                                    ySum_alpha +=
                                            totalWeight * inputTensorData[neighbourOffset + i * desc.channelStride] *
                                            yDistanceSquared[kernelIndex[1]] / std::abs(sigma_y * sigma_y * sigma_y);

                                    zSum_w +=
                                            totalWeight * zDistanceSquared[kernelIndex[2]] /
                                            std::abs(sigma_z * sigma_z * sigma_z);
                                    zSum_alpha +=
                                            totalWeight * inputTensorData[neighbourOffset + i * desc.channelStride] *
                        ...
                        for (int i = 0; i < desc.channelCount; i++) {
                            ...
                            dO_dx_kiData[homeOffset + i * desc.channelStride] =
